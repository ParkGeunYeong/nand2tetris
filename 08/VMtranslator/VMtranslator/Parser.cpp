...
int Parser::commandType()
{
	...
	if (instruction.substr(0, 5) == "label") return C_LABEL;
	if (instruction.substr(0, 4) == "goto") return C_GOTO;
	if (instruction.substr(0, 7) == "if-goto") return C_IF;
}
...
string Parser::arg1()
{
	...
	if (commandType() == C_LABEL) return "null$"+instruction.substr(5, string::npos);
	if (commandType() == C_GOTO) return instruction.substr(4, string::npos);
	if (commandType() == C_IF) return instruction.substr(7, string::npos);
}
